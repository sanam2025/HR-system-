...
export default function HRLayout() {
  ...
  const hrSideBar = [
    ...
    { path: "all-applicants", label: "Applicants", icon: UsersRound, exact: false },
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-100">
      <Sidebar navItems={hrSideBar} onToggle={() => setOpen(!open)} open={open} />
      <main className={`flex-1 overflow-y-auto transition-all duration-300 ${open ? "ml-64" : "ml-20"}`}>
        <div className="p-6">
          <Outlet />
        </div>
      </main>
