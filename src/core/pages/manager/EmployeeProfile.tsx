...
import { ArrowRight, Phone, Mail, Calendar, Star, CheckSquare, Clock } from 'lucide-react';
...
export default function EmployeeProfile() {
  ...
  if (!employee) return (
    <div className="text-center py-20 text-gray-400">
      ...
      <button onClick={() => navigate('/manager/employees')} className="btn-primary btn mt-4">رجوع</button>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
          ...
          <div className="flex items-center gap-2 text-sm text-brown">
            <Calendar size={15} className="text-gold" />تاريخ الانضمام: {employee.joinDate}
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl p-5 text-center bg-gold/10 text-yellow-800">
          ...
          <p className="text-xl font-extrabold">{employee.leaveBalance} يوم</p>
          <p className="text-xs font-semibold mt-1 opacity-80">رصيد الإجازات</p>
        </div>
        <div className="rounded-2xl p-5 text-center bg-green/10 text-green-700">
          ...
          <p className="text-xs font-semibold mt-1 opacity-80">إجمالي المهام</p>
        </div>
        <div className="rounded-2xl p-5 text-center bg-brown/10 text-brown">
          <div className="text-3xl mb-2">⭐</div>
          <p className="text-xl font-extrabold">{avgRatingDisplay} ★</p>
          <p className="text-xs font-semibold mt-1 opacity-80">متوسط التقييم</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-card">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="font-bold text-dark flex items-center gap-2">
            <CheckSquare size={16} className="text-green" /> المهام الحالية
          </h3>
        </div>
        {empTasks.length === 0 ? (
          <p className="text-center text-gray-400 py-10 text-sm">لا توجد مهام</p>
        ) : (
            ...
            {empTasks.map(task => (
              <div key={task.id} className="flex items-center justify-between px-6 py-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">تاريخ الاستحقاق: {task.dueDate}</p>
                </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-card">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="font-bold text-dark flex items-center gap-2">
            <Clock size={16} className="text-gold" /> سجل الحضور
          </h3>
        </div>
        <div className="divide-y divide-gray-50">
