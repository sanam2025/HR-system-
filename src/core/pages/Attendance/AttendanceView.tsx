...
const getEmployeeAttendance = (empId: number): AttendanceRecord[] => {
  ...
  if (empId === 1) {
    result = base;
  } else if (empId === 2) { // Sara: always present, no delays
    result = base.map(r => r.status === 'غائب' || r.status === 'تأخير'
      ? { ...r, status: 'حاضر', checkIn: '08:00', checkOut: '17:00', delay: 0, earlyLeave: 0 }
      : r
    );
  } else if (empId === 3) { // Mohamed: high absence rate
    result = base.map((r, idx) => idx % 2 === 0
      ? { ...r, status: 'غائب', checkIn: null, checkOut: null, delay: 0, earlyLeave: 0 }
      : r
    );
  } else if (empId === 4) { // Layla: present with some early leaves
    result = base.map((r, idx) => r.status === 'غائب'
      ? { ...r, status: 'حاضر', checkIn: '08:00', checkOut: '17:00', delay: 0, earlyLeave: 0 }
      : idx === 3 ? { ...r, earlyLeave: 45 } : r
    );
  } else if (empId === 5) { // Khalid: high lateness rate
    result = base.map((r, idx) => idx % 2 === 1
      ? { ...r, status: 'تأخير', checkIn: '08:45', checkOut: '17:00', delay: 45 }
      : r
    );
  } else {
    // Nour: app developer, mostly present
    result = base.map((r, idx) => idx === 0 ? { ...r, status: 'حاضر', checkIn: '08:00', delay: 0 } : r);
  }

  // Post-process: If delay is greater than 0, status should be 'تأخير' (Late)
  return result.map(r => {
    if (r.delay && r.delay > 0 && (r.status === 'حاضر' || r.status === 'Present')) {
      return { ...r, status: 'تأخير' };
    }
    return r;
  });
};
...
export default function AttendanceView() {
  ...
  // Aggregate and filter all attendance records for general report
  ...
  const filteredRecords = allRecords.filter(r => {
    ...
    if (statusFilter === 'present') return r.status === 'حاضر' || r.status === 'Present';
    if (statusFilter === 'absent') return r.status === 'غائب' || r.status === 'Absent';
    if (statusFilter === 'late') return r.status === 'تأخير' || r.status === 'Late';
    return true;
  });

  // Calculate statistics correctly based on active tab
  const statsRecords = activeTab === 'byEmployee'
    ? attendanceRecords
    : allRecords.filter(r => r.date >= startDate && r.date <= endDate);

  const present = statsRecords.filter(r => r.status === 'حاضر' || r.status === 'Present').length;
  const absent = statsRecords.filter(r => r.status === 'غائب' || r.status === 'Absent').length;
  const late = statsRecords.filter(r => r.status === 'تأخير' || r.status === 'Late').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-gray-200">
        <button
          ...
          className={`pb-2.5 px-4 font-bold text-sm border-b-2 transition-all ${activeTab === 'byEmployee'
        ...
        <button
          ...
          className={`pb-2.5 px-4 font-bold text-sm border-b-2 transition-all ${activeTab === 'generalReport'
            ? 'border-green text-green font-extrabold'
            : 'border-transparent text-gray-400 hover:text-dark'
            }`}
        >
      ...
      {activeTab === 'generalReport' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-5 flex flex-wrap gap-6 items-end">
          <div className="flex-1 min-w-[200px] text-start">
            <label className="block text-xs font-bold text-brown uppercase mb-2">
              {t.attendance.filter.fromDate}
            </label>
            <input
              type="date"
              value={startDate}
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold border transition-all ${statusFilter === st.id
            ...
            <div className="space-y-1 max-h-[400px] overflow-y-auto pr-1">
              {filteredEmployees.map(emp => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-start transition-all ${selectedEmp === emp.id
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {attendanceRecords.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center py-12 text-gray-400 text-start">
