...
export default function Employees() {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">{t.hrEmployees?.title || 'Employees & Departments'}</h1>
        <p className="text-gray-500 text-sm">{t.hrEmployees?.subtitle || 'Manage employees and browse departments.'}</p>
      </div>
            ...
            <div className="p-4 border-b border-gray-100 flex justify-between items-center">
              <div>
                ...
                {department.manager_name && (
                  <p className="text-xs text-gray-500">{t.hrEmployees?.manager || 'Manager:'} {department.manager_name}</p>
                )}
              </div>
              <ChevronRight className={`w-4 h-4 text-gray-400 ${lang === 'ar' ? 'rotate-180' : ''}`} />
            </div>
            <div className="p-4">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <Users className="w-4 h-4" />
                <span>{department.employees?.length || 0} {t.hrEmployees?.employeesCount || 'employees'}</span>
              </div>
