...
export default function ComplaintDetail() {
  ...
  const handleRespond = () => {
    if (!responseText.trim()) {
      toast.error(isRTL ? 'الرجاء إدخال الرد' : 'Please enter a response');
      return;
    }
  ...
  if (!complaint) return <p className="text-red-500">{isRTL ? 'الشكوى غير موجودة' : 'Complaint not found'}</p>;
  ...
  return (
    <div className={`p-6 bg-gray-50 min-h-screen ${isRTL ? 'font-cairo' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
      <button onClick={() => navigate('/Hr/complaints')} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 mb-3">
        <ArrowLeft className={`w-4 h-4 ${isRTL ? 'rotate-180' : ''}`} /> {isRTL ? 'العودة للشكاوى' : 'Back to Complaints'}
      </button>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="mb-6 text-sm text-gray-600">
          <p><span className="font-medium">{isRTL ? 'المشتكي:' : 'Complainant:'}</span> {complaint.author?.full_name || (isRTL ? 'غير معروف' : 'Unknown')}</p>
          <p><span className="font-medium">{isRTL ? 'ضد:' : 'Against:'}</span> {complaint.subject?.full_name || (isRTL ? 'غير معروف' : 'Unknown')}</p>
        </div>

        <div className="mb-8">
          <h3 className="text-sm font-semibold text-gray-700 mb-2">{isRTL ? 'الوصف' : 'Description'}</h3>
          <p className="text-gray-600 bg-gray-50 p-4 rounded-lg">{complaint.description}</p>
        </div>

        <div className="border-t pt-6">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">{isRTL ? 'رد الموارد البشرية' : 'HR Response'}</h3>
          {complaint.status === 'pending' || complaint.status === 'under_review' ? (
            <>
              ...
              <div className="flex gap-4 items-center flex-wrap">
                ...
                >
                  ...
                  {respondMutation.isPending ? (isRTL ? 'جاري الإرسال...' : 'Sending...') : (isRTL ? 'إرسال الرد' : 'Send Response')}
                </button>
                {complaint.status === 'pending' && (
                  ...
                  >
                    {markUnderReviewMutation.isPending ? (isRTL ? 'جاري التحديث...' : 'Updating...') : (isRTL ? 'تحديد كقيد المراجعة' : 'Mark as Under Review')}
                  </button>
                )}
              </div>
            </>
            ...
            <div className="text-gray-600 bg-gray-50 p-4 rounded-lg">
              {complaint.hr_note || (isRTL ? 'لم يتم تقديم رد من الموارد البشرية.' : 'No HR response provided.')}
            </div>
          )}
        </div>
      </div>
