    ...
    <div className="p-6 bg-gray-50 min-h-screen" dir="ltr">
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-lg">
          <button onClick={() => setActiveTab('all')} className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'all' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>All Contracts</button>
          <button onClick={() => setActiveTab('expiring')} className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'expiring' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>Expiring Soon <span className="ml-1 text-xs text-red-500">({expiringContracts.length})</span></button>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Contract #</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Employee</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Department</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Position</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Period</th>
                ...
                <tr key={contract.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-gray-900">{contract.employee?.full_name || 'N/A'}</span>
                  ...
                  <td className="px-5 py-4 text-sm text-gray-600">{contract.start_date} → {contract.end_date}</td>
                  <td className="px-5 py-4 text-right flex items-center justify-end gap-2">
                    ...
                    {activeTab === 'expiring' && (
                      <>
                        <button onClick={() => handleRenew(contract.id)} className="px-3 py-1 bg-emerald-600 text-white rounded-md text-xs">Renew</button>
                        <button onClick={() => handleNonRenew(contract.id)} disabled={nonRenewMutation.isPending} className="px-3 py-1 bg-red-600 text-white rounded-md text-xs">Non-Renew</button>
                      </>
                    )}
                  </td>
                </tr>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6 border-t pt-4">
                <button onClick={() => setRenewContractId(null)} className="px-4 py-2 border rounded-lg hover:bg-gray-50">Cancel</button>
                <button onClick={confirmRenew} disabled={renewMutation.isPending} className="px-5 py-2 bg-emerald-600 text-white rounded-lg">Confirm Renewal</button>
              </div>
            </div>
          </div>
        </div>
